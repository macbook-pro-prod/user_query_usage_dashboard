...
CREATE OR REPLACE TABLE `prod-dw-id.ds_dwh01_shared.jobs_by_project`
...
FROM `prod-dw-id.region-asia-southeast1.INFORMATION_SCHEMA.JOBS_BY_PROJECT` AS j

WHERE total_bytes_processed <> 0
  AND state = 'DONE'

  -- Initial backfill: previous 175 completed days
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM `prod-dw-id.region-asia-southeast1.INFORMATION_SCHEMA.JOBS_BY_PROJECT` AS child
