...
ALTER TABLE `prod-raw-landing.ds_dwh01_shared.jobs_by_project` 
...
DELETE FROM `prod-raw-landing.ds_dwh01_shared.jobs_by_project` -- change to certain project to backfilling
...
INSERT INTO `prod-raw-landing.ds_dwh01_shared.jobs_by_project` -- change to certain project to backfilling
(
  start_time,
  user_email,
  job_type,
...
FROM `prod-raw-landing.region-asia-southeast1.INFORMATION_SCHEMA.JOBS_BY_PROJECT` -- change to certain project to backfilling
...
WHERE total_bytes_processed <> 0
  ...
  AND job_id NOT IN (
    ...
    FROM `prod-raw-landing.ds_dwh01_shared.jobs_by_project` -- change to certain project to backfilling
